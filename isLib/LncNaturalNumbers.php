...
class LncNaturalNumbers {
    ...
    public function nnAdd(array $u, array $v):array {
        ...
        for ($i = 1; $i <= $length; $i++) {
            ...
            $sum[$i] = self::mmMod($machineSum, $this->radix);
    ...
    public function nnSub(array $u, array $v):array {
        ...
        for ($i = 1; $i <= $length; $i++) {
            ...
            $diff[$i] = self::mmMod($machineDiff, $this->radix);
            if ($machineDiff < 0) {
                $borrow = -1;
            } else {
                $borrow = 0;
        ...
        for ($j = 1; $j <= $v[0]; $j++) {
            if ($v[$j] != 0) {
                ...
                for ($i = 1; $i <= $u[0]; $i++) {
                    ...
                    $w[$i + $j -1] = self::mmMod($machineProduct, $this->radix);
                    $carry = self::mmDiv($machineProduct, $this->radix);
                }
                $w[$u[0] + $j] = $carry;
            }
        }
    ...
    public function nnShortDivMod(array $u, int $v):array {
        ...
        $r = 0;
        for ($i = $u[0]; $i >= 1; $i--) {
            $quotient[$i] = self::mmDiv($u[$i] + $this->radix * $r, $v);
            $r = self::mmMod($u[$i] + $this->radix * $r, $v);
        }
    ...
    public function nnDivMod(array $u, array $v):array {
        ...
        $d = self::mmDiv($this->radix, $v[$v[0]] + 1); 
        ...
        while (!$done) {
            ...
            if ($m >= 0) {
                ...
                if (!$done) {
                    ...
                    $den = $vn[$vn[0]];
                    $trialQuotient = self::mmdiv($num, $den);
    ...
    /********************************************************
     * These functions are native to the underlying machine *
     ********************************************************/
